...
public class Functions {
    ...
    public String getColumn(Cursor crs, String columnName){
        return crs.getString(crs.getColumnIndexOrThrow(columnName));
    }
    ...
    public void autoSync(){
        db.open();
        String lastSyncedTblPost = db.getLastSync("tbl_post");
        String lastSyncedTblImg = db.getLastSync("tbl_image");
        String lastSyncedTblPtyType = db.getLastSync("tbl_pty_type");
        db.close();

        try {
            String response = new RequestHandler().postJSON(endPoint("auto_sync"),
                    new JSONObject()
                            .put("tbl_post_ls", lastSyncedTblPost)
                            .put("tbl_image_ls", lastSyncedTblImg)
                            .put("tbl_pty_type_ls", lastSyncedTblPtyType));
            // Log.e("AUTO_SYNC", response);
            JSONObject jTables = new JSONObject(response);
            replicateTable("tbl_post", jTables.getJSONObject("tbl_post"));
            replicateTable("tbl_image", jTables.getJSONObject("tbl_image"));
            replicateTable("tbl_pty_type", jTables.getJSONObject("tbl_pty_type"));
    ...
    public String endPoint(String fileName){
        return String.format("http://www.movein.co.ke/app_endpoints/%s.php",fileName);
    }

    public boolean isCleanString(EditText et){
    ...
    public String cleanMSISDN(String msisdn){

        StringBuilder sb = new StringBuilder(msisdn.replaceAll("\\s",""));
        sb.replace(0,1,"+254");

        return sb.toString();
    }

    public void populateSpinner(Spinner spinner, Cursor cursor){
        List<String> entries = new ArrayList<>();
        entries.add("Select");

        for(int i=0; i<cursor.getCount(); i++) {
            cursor.moveToPosition(i);
            entries.add(cursor.getString(cursor.getColumnIndexOrThrow("name")));
        }

        ArrayAdapter<String> adapter = new ArrayAdapter<>(
                context, R.layout.spinner_item, entries);
        //adapter.setDropDownViewResource(R.layout.spinner_item);
        spinner.setAdapter(adapter);
    }

    public int readSpinnerInt(Cursor crs, Spinner spinner){
        crs.moveToPosition(spinner.getSelectedItemPosition() - 1);
        return crs.getInt(crs.getColumnIndexOrThrow("id"));
    }

    public String readSpinnerString(Cursor crs, Spinner spinner){
        crs.moveToPosition(spinner.getSelectedItemPosition() - 1);
        return crs.getString(crs.getColumnIndexOrThrow("name"));
    }

    public String uploadStringImage(String imagePath){
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inSampleSize = 4;
        try {
            //Bitmap bitmap = BitmapFactory.decodeFile(resizeImage(imagePath), options);
            Bitmap bitmap = BitmapFactory.decodeFile(imagePath, options);
            return getStringImage(getBytes(bitmap));
        }catch (Exception ex){
            return "";
        }
    }
    ...
    void replicateTable(String tableName, JSONObject data){
        try {
            if (data.getInt("success") == 1) {
                ...
                try {
                    ...
                    for (int i = 0; i < rows.length(); i++) {
                        ...
                        try {
                            db.addEntry(tableName, row);
                        } catch (SQLiteConstraintException ex) {
                            db.updateEntry(tableName, row);
                        }
                    }
    ...
    void consts() {
        ...
        String[] images = new String[]{
                "https://scontent-sit4-1.xx.fbcdn.net/hphotos-xfa1/v/t1.0-9/12919745_767869349981904_7785260240145072896_n.jpg?oh=98b6f96524b30ef294e5caeaf88778b3&oe=578F916F",
                "https://scontent-sit4-1.xx.fbcdn.net/hphotos-xlp1/v/t1.0-9/12936543_767869856648520_5380554800863102221_n.jpg?oh=6ee869be2c5b6df665b5153e99122638&oe=577592E0",
